...
range returns an [iterator](https://docs.python.org/3/glossary.html#term-iterator) that will yield the integers up to the given stop or from the given start up to the given stop.
...
Be aware that the range object is an iterator, so it will be consumed.
...
You will see the range object most commonly used in the for loops, for when you want an index instead of itering through an iterable object.
