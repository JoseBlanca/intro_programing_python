...
If you run the previous piece you won't see any result. Once you define a function, the function will be ready to be used, but in order to use it you need to call it.
...
The lines that belong to the function are the lines indented after the function definition line, that's the lines in the [block](../control_flow/block.qmd).
...
As a general rule the use of the global scope is discouraged. Try not to use the global scope, access to the variables.
...
If we want to pass data to the function we have to use the arguments in the function definition, and if we want to get data out of the function we use the [return](https://docs.python.org/3/reference/simple_stmts.html#the-return-statement) statement.
...
You can think on a method like on a function that has a its first argument the given object.
